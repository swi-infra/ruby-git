...
> **⚠️ Work in progress — v5.0 is currently in beta.**
...
The primary goal of v5.0 is to move everyone onto a new internal architecture
while keeping the existing v4.x API working. The vast majority of v4.x code
requires no changes to run on v5.0.

The new architecture delivered in v5.0 is the foundation for future API
...
eliminated in a future major release (most likely v6.0).

Hard breaks are limited to a small number of internal methods that had no safe
migration path — these are described in detail below.

Our intent is to make upgrading to v5.0 as smooth as possible. For information
...
> **Note:** The changes described in this section are planned for v5.0.0 final
> and are being implemented incrementally through the beta cycle. The current
> beta may not yet have all replacement methods and deprecation warnings in place.
...
implementation detail that leaked out. `Git::Lib` will be removed in v5.0.0.

In v5.0.0, calling `#lib` on the repository object will return `self` with a
deprecation warning so that existing `g.lib.*` call chains continue to work
during migration. The `#lib` method itself will be removed in v6.0. Calls to
methods that have no replacement (see below) will raise `NoMethodError` with an
informative message.

All public behavior previously accessible via `g.lib.*` will be available
directly on the repository object (i.e., `g.*`). The sections below list
every affected method.

#### Methods with a direct replacement

All of the following `g.lib.*` calls will work in v5.0.0 but emit a deprecation
warning. Migrate to the replacement shown to silence the warning. Some
replacement methods listed may not yet be available in the current beta.

| Deprecated call (works in v5.0, removed in v6.0) | Replacement |
|---------------------------------------------------|-------------|
...
helpers with no plausible external use. They have no replacement in v5.0:
...
The following methods will be available in v5.0.0 with deprecation warnings
and will be removed in v6.0. Some may not yet be available in the current beta.

...
| `g.branch(name).stashes.all` | `g.stash_infos` |
...
> message git stores, and `Git::StashInfo#branch` holds the branch name (`nil` when
> the message has no branch prefix, as for a stash created from a detached HEAD or
> with `stash_store`).
...
| `g.stashes_all.each { \|index, message\| ... }` | `g.stash_infos.reverse_each { \|info\| ... info.message }` |
...
| `Git::Stash.new(g, message)` | `g.stash_push(message: message)` |
| `Git::Stash.new(g, message, existing: true)` | `g.stash_infos.find { \|s\| s.message.end_with?(message) }` |
| `stash.save` | `g.stash_push(message: message)` |
| `stash.saved?` | `!g.stash_push(message: message).nil?` |
| `stash.message` / `stash.to_s` | `info.message` — keeps the branch prefix; see the note above |
| `Git::Stashes.new(g)` | `g.stash_infos` |
| `stashes.all` | `g.stash_infos` |

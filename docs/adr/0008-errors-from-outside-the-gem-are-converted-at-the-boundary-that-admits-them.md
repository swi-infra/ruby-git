...
`Git::Error`. The rule that keeps the promise: any other class that a standard library
or gem call can raise is converted at the boundary where it enters, to whichever of the
two contract classes fits. A caller mistake becomes `ArgumentError`. A failure at run
time becomes `Git::Error` or a subclass, with the underlying error as `cause`. The
subprocess layer converts process_executer's errors this way, the parsers convert a
malformed date from `Time.iso8601`, and `Git::SystemCallGuard` converts
...
where a facade method such as `chdir` yields to the caller.
...
The exception class follows the kind of failure, not the path. Validating an argument
raises `ArgumentError`, and a filesystem operation that fails raises `Git::Error`, even
when both stem from the same missing directory. `with_working` does both in turn.

...
RSpec.describe Git::VersionConstraint do
  describe 'instantiation' do
    ...
    it 'defaults both to nil when no arguments given' do
      constraint = described_class.new
      expect(constraint.min).to be_nil
      expect(constraint.before).to be_nil
    end
  ...
  describe '#satisfied_by?' do
    ...
    context 'with no constraints (both nil)' do
      let(:constraint) { described_class.new }

      it 'returns true for any version' do
        expect(constraint.satisfied_by?(Git::Version.parse('1.0.0'))).to be true
        expect(constraint.satisfied_by?(Git::Version.parse('99.0.0'))).to be true
      end
    end
  end

...
RSpec.describe Git::Lib do
  ...
  describe '#clone' do
    ...
    before do
      ...
      allow(clone_command).to receive(:call).and_return(command_result)
    end
    ...
    context 'with nil directory' do
      it 'derives directory from URL' do
    ...
    context 'with :log option' do
      it 'includes log in the result' do
        ...
        expect(clone_command).to have_received(:call).with(repository_url, directory, log: logger)
    ...
    context 'with :git_ssh option' do
      it 'includes git_ssh in the result' do
        ...
        expect(clone_command).to have_received(:call).with(repository_url, directory, git_ssh: 'ssh -i /path/to/key')
      ...
      it 'includes git_ssh even when nil' do
        ...
        expect(clone_command).to have_received(:call).with(repository_url, directory, git_ssh: nil)
    ...
    context 'with multiple options' do
      it 'includes all result fields' do
        ...
        expect(clone_command).to have_received(:call)
          .with(repository_url, directory, log: logger, git_ssh: 'custom-ssh')
        expect(result).to eq(
      ...
      it 'combines bare, log, and git_ssh' do
        ...
        expect(clone_command).to have_received(:call)
          .with(repository_url, directory, bare: true, log: logger, git_ssh: nil)
        expect(result).to eq(

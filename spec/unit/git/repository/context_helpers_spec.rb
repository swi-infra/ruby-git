...
RSpec.describe Git::Repository::ContextHelpers do
  ...
  describe '#with_temp_index' do
    ...
    it 'cleans up the temporary file after the block succeeds' do
      temp_path = nil
      described_instance.with_temp_index { temp_path = described_instance.index.to_s }
      expect(temp_path).not_to be_nil
      expect(File.exist?(temp_path)).to be(false)
    end

    it 'cleans up the temporary file even when the block raises' do
      temp_path = nil
      expect do
        described_instance.with_temp_index do
          temp_path = described_instance.index.to_s
          raise 'block error'
        end

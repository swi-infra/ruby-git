...
RSpec.describe Git::Repository::Committing do
  ...
  describe '#commit_tree' do
    ...
    context 'with a tree and no options' do
      ...
      it 'delegates with a default message derived from the tree' do
        expect(commit_tree_command).to(
          receive(:call)
            .with('tree-sha', message: 'commit tree tree-sha')
    ...
    context 'with a message option' do
      ...
      it 'passes :message directly to the command' do
        expect(commit_tree_command).to receive(:call).with('tree-sha',
                                                           message: 'my message').and_return(commit_tree_result)
        result
      end
    end
    ...
    context 'with :parent option' do
      ...
      it 'passes :parent directly to the command' do
        expect(commit_tree_command).to receive(:call).with('tree-sha', parent: 'parent-sha',
                                                                       message: 'msg').and_return(commit_tree_result)
        result
      end
    end
    ...
    context 'with :parents option (array)' do
      ...
      it 'normalizes :parents to :parent' do
        expect(commit_tree_command).to receive(:call).with('tree-sha', parent: %w[sha1 sha2],
                                                                       message: 'msg').and_return(commit_tree_result)
        result
      end
    end
  ...
  describe '#write_and_commit_tree' do
    ...
    context 'with no options' do
      ...
      it 'calls write_tree then commit_tree in order' do
        ...
        expect(commit_tree_command).to(
          receive(:call)
          .with(tree_sha, message: "commit tree #{tree_sha}")
    ...
    context 'with message option' do
      ...
      it 'forwards options to commit_tree' do
        ...
        expect(commit_tree_command).to receive(:call).with(tree_sha,
                                                           message: 'my commit').and_return(command_result(commit_sha))
        result
      end
    end
  end

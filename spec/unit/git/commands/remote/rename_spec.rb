  ...
  describe '#call' do
    ...
    context 'with :progress option' do
      it 'includes --progress when true' do
        expect_command_capturing('remote', 'rename', '--progress', 'origin', 'upstream').and_return(command_result)

        command.call('origin', 'upstream', progress: true)
      end

      it 'includes --no-progress when false' do
        expect_command_capturing('remote', 'rename', '--no-progress', 'origin', 'upstream').and_return(command_result)

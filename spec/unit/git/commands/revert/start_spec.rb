  ...
  describe '#call' do
    context 'with a single commit' do
      it 'calls git revert with the commit ref' do
        ...
        expect_command_capturing('revert', '--no-edit', '--', 'abc123').and_return(expected_result)
    ...
    context 'with multiple commits' do
      it 'includes all commit refs' do
        expect_command_capturing('revert', '--no-edit', '--', 'abc123', 'def456').and_return(command_result(''))
    ...
    context 'with :cleanup option' do
      it 'adds --cleanup=<mode>' do
        expect_command_capturing('revert', '--no-edit', '--cleanup=strip', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :no_commit option' do
      it 'adds --no-commit when true' do
        expect_command_capturing('revert', '--no-edit', '--no-commit', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :gpg_sign option' do
      it 'adds --gpg-sign when true' do
        expect_command_capturing('revert', '--no-edit', '--gpg-sign', '--', 'abc123').and_return(command_result(''))
        command.call('abc123', gpg_sign: true)
      end

      it 'adds --no-gpg-sign when false' do
        expect_command_capturing('revert', '--no-edit', '--no-gpg-sign', '--', 'abc123').and_return(command_result(''))
        command.call('abc123', gpg_sign: false)
      end

      it 'adds --gpg-sign=<keyid> when given a key ID string' do
        expect_command_capturing('revert', '--no-edit', '--gpg-sign=ABCDEF01', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :signoff option' do
      it 'adds --signoff when true' do
        expect_command_capturing('revert', '--no-edit', '--signoff', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :strategy option' do
      it 'adds --strategy=<name>' do
        expect_command_capturing('revert', '--no-edit', '--strategy=ort', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :strategy_option option' do
      it 'adds --strategy-option=<value>' do
        expect_command_capturing('revert', '--no-edit', '--strategy-option=theirs', '--', 'abc123').and_return(command_result(''))
        command.call('abc123', strategy_option: 'theirs')
      end

      it 'supports multiple strategy options as an array' do
        expect_command_capturing(
          'revert', '--no-edit', '--strategy-option=ours', '--strategy-option=patience', '--', 'abc123'
    ...
    context 'with :rerere_autoupdate option' do
      it 'adds --rerere-autoupdate when true' do
        expect_command_capturing('revert', '--no-edit', '--rerere-autoupdate', '--', 'abc123').and_return(command_result(''))
        command.call('abc123', rerere_autoupdate: true)
      end

      it 'adds --no-rerere-autoupdate when false' do
        expect_command_capturing('revert', '--no-edit', '--no-rerere-autoupdate', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :reference option' do
      it 'adds --reference when true' do
        expect_command_capturing('revert', '--no-edit', '--reference', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with :mainline option' do
      it 'adds --mainline <n>' do
        expect_command_capturing('revert', '--no-edit', '--mainline', '1', '--', 'abc123').and_return(command_result(''))
        command.call('abc123', mainline: 1)
      end

      it 'supports the :m alias' do
        expect_command_capturing('revert', '--no-edit', '--mainline', '2', '--', 'abc123').and_return(command_result(''))
    ...
    context 'with multiple options combined' do
      it 'includes all specified flags in the correct order' do
        expect_command_capturing(
          'revert', '--no-edit', '--no-commit', '--signoff', '--', 'abc123', 'def456'
        ).and_return(command_result(''))

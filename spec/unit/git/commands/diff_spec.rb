  ...
  describe '#call' do
    ...
    context 'with :l option' do
      it 'includes -l <num>' do
        expect_command_capturing('diff', '-l', '1000').and_return(command_result(''))
        command.call(l: 1000)
      end
    end
    ...
    context 'with :S option' do
      it 'includes -S <string>' do
        expect_command_capturing('diff', '-S', 'search_term').and_return(command_result(''))
    ...
    context 'with :G option' do
      it 'includes -G <regex>' do
        expect_command_capturing('diff', '-G', 'pattern').and_return(command_result(''))

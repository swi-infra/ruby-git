...
RSpec.describe Git::Repository::Stashing, :integration do
  ...
  describe '#stash_store' do
    ...
    context 'when the commit cannot be stored' do
      it 'raises ArgumentError without running git when the commit is nil' do
        expect { described_instance.stash_store(nil) }.to raise_error(ArgumentError, 'commit is required')
        expect(described_instance.stash_list).to eq([])
      end
      ...
      it 'raises Git::FailedError without storing anything when the commit looks like an option' do
        expect { described_instance.stash_store('-x') }.to raise_error(Git::FailedError, /Needed a single revision/)
        expect(described_instance.stash_list).to eq([])
      end

...
RSpec.describe Git::Commands::Pull, :integration do
  ...
  let(:second_clone_dir) { File.join(Dir.tmpdir, "second_clone_#{Process.pid}_#{rand(1000)}") }
  ...
  describe '#call' do
    ...
    context 'when pulling from a named remote and branch' do
    ...
    context 'when pulling a specific remote and branch a second time' do
      it 'succeeds and returns a CommandLineResult when already up to date' do
        # First pull to bring the repo up to date
        command.call('origin', 'main')
        # Second pull when already up to date also succeeds
        result = command.call('origin', 'main')

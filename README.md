...
with the `raise` behavior described under [Deprecations](#deprecations), a deprecated
call raises `ActiveSupport::DeprecationException`, which is intentionally not a
`Git::Error` so a broad rescue cannot hide a deprecation your code opted to treat as
fatal.

...
| `allow_nil: true` | The argument is optional, nil is a meaningful distinct value from "not provided", and callers need to explicitly pass `nil` to express that |
...
**`allow_nil: true` on `required:` arguments:** flag `allow_nil: true` as suspicious
when combined with `required: true` — allowing nil on a required argument is rarely
correct.

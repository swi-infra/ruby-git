...
3. **Exclude it with `# :nocov:`** — the last resort, per the exception below.
...
> — never leave branches silently uncovered. Wrap the smallest possible span, and
> expect a reviewer to challenge it. `lib/` currently contains no `# :nocov:` markers.

...
Every individual YARD tag must be preceded by a blank comment line (`#`). A YARD
tag is any comment token matching `@!?[a-z_]+` — that is, `@word` (regular tags
such as `@param`, `@return`, `@raise`, `@api`, `@abstract`, `@deprecated`, etc.)
or `@!word` (directives such as `@!attribute`, `@!method`, `@!scope`, etc.).

There are no exceptions: consecutive same-kind tags (e.g. multiple `@param` lines)
each require their own preceding blank line.
...
Never use back-tick shell calls (`` `true` ``, `` `git version` ``) or process-status

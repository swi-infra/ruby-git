...
than two separate entries. This registers both `:foo` (positive) and `:no_foo`
...
sets safe policy defaults at each call site — `no_edit: true`, `verbose: true`,
`no_progress: true`, etc. — not as `literal` entries inside the command class. Callers

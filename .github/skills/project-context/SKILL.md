...
- Any class other than `ArgumentError` that a standard library or gem call can raise
  is converted at that call, to `ArgumentError` for a caller mistake or to
  `Git::Error` (or a subclass) otherwise, with the underlying error as `cause`. No
  site is exempt because its failure is unlikely or because the caller chose the path.
...
- Parsers convert the `ArgumentError` from `Time.iso8601` to
  `Git::UnexpectedResultError`. Subprocess errors are converted in
  `Git::CommandLine`; command classes do not convert them again.

...
`requires_one_of :object, :batch_all_objects`. `:object` is `skip_cli: true` — git
never sees it, only `:batch_all_objects` reaches argv. Git cannot detect that you passed
both (silent wrong result: dumps the entire object database) or neither (empty output
with exit 0), so Ruby must enforce those constraints.
...
behavior was verified, and a test. No such case has been identified so far.

One command deviates from this rule today: `Git::Commands::CatFile::Raw` declares

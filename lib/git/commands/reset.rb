...
module Git
  module Commands
    ...
    class Reset < Git::Commands::Base
      ...
      #
      #     @option options [Boolean] :soft (nil) leave working tree and index unchanged;
      #       reset HEAD to the specified commit
      #
      #     @option options [Boolean] :mixed (nil) reset the index but not the working tree;
      #       default mode when no mode flag is given
      #
      #     @option options [Boolean] :N (nil) mark removed paths as intent-to-add;
      #       only meaningful alongside `:mixed`
      #
      #     @option options [Boolean] :hard (nil) reset the index and working tree to the
      #       specified commit; discards all tracked changes since that commit
      #
      #     @option options [Boolean] :merge (nil) reset the index and update files that
      #       differ between the commit and HEAD, while preserving uncommitted changes
      #
      #     @option options [Boolean] :keep (nil) reset index entries and update files that
      ...
      #
      #     @option options [Integer] :unified (nil) number of context lines around each diff hunk
      #
      #       Alias: :U
      #
      #     @option options [Integer] :inter_hunk_context (nil) number of context lines to show
      #       between diff hunks
      #
      #     @option options [String] :pathspec_from_file (nil) read pathspec from the given file;
      #       pass `"-"` to read from standard input
      #
      #     @option options [Boolean] :pathspec_file_nul (nil) delimit pathspec elements with NUL

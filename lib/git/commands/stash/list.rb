...
module Git
  module Commands
    module Stash
      ...
      #   stashes = Git::Commands::Stash::List.new(context).call
      #   stashes.each do |s|
      ...
      class List
        ...
        #
        def call
          lines = @context.command_lines('stash', 'list', "--format=#{STASH_FORMAT}")
          lines.each_with_index.map { |line, idx| parse_stash_line(line, idx) }.compact
        end
        ...
        #
        # @return [Git::StashInfo, nil] parsed stash info or nil if line doesn't match
        #
        def parse_stash_line(line, expected_index)
          parts = line.split(FIELD_SEPARATOR, FIELD_COUNT)
          return nil unless parts.length == FIELD_COUNT

          build_stash_info(parts, expected_index)
        end

...
module Git
  class Repository
    ...
    module ContextHelpers
      ...
      #
      # The temporary file is removed unconditionally after the block exits,
      # even if the block raises an exception. The file is allocated with
      # `Tempfile.new` so the path is atomically reserved before use.
      #
      ...
      def with_temp_index(&) # :yields: self
        # Tempfile.new allocates an exclusively-created path (CREAT|EXCL internally).
        # Close the fd so git can write to it, then delete in ensure.
        temp_file = Tempfile.new('temp-index')
        temp_file.close
        begin
          with_index(temp_file.path, &)
        ensure
          temp_file.close!
        end
      end
      ...
      def context_helpers_deprecate_check_argument(check, must_exist)
        ...
        return must_exist unless must_exist.nil?

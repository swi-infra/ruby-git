...
module Git
  class Repository
    ...
    module RemoteOperations
      ...
      #   (`--mirror`). When `:tags` is also given, the separate tags push is suppressed.
      #
      # @option options [Boolean, nil] :tags (nil) push all refs under `refs/tags/` in a
      #   second `git push` invocation (`--tags`). When `:mirror` is also given, the tags
      #   push is suppressed because `--mirror` already includes tags.
      #
      # @option options [Boolean, nil] :force (nil) force updates, overriding the fast-forward
      #   check (`--force`)
      #
      #   Alias: `:f`
      #
      # @option options [Boolean, nil] :delete (nil) delete the named refs from the remote
      #   (`--delete`)
      #
      # @option options [String, Array<String>] :push_option (nil) one or more server-side
      #   push option values (`--push-option=<value>`, repeatable)
      #
      # @raise [ArgumentError] when unsupported option keys are provided
      #
      # @raise [Git::FailedError] when git exits with a non-zero status

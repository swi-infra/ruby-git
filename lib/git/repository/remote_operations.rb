...
module Git
  class Repository
    ...
    module RemoteOperations # rubocop:disable Metrics/ModuleLength
      ...
      # @raise [Git::FailedError] if git exits with a non-zero exit status
      #
      # @deprecated Use {#remote_list} instead. Note that {#remote_list} returns
      #   `Array<Git::RemoteInfo>` (immutable value objects) rather than
      #   `Array<Git::Remote>`; call the corresponding {Git::Repository} method
      #   (e.g. {#fetch}, {#remote_remove}) for operations on a remote.
      #
      # @see #remote_list
      #
      def remotes

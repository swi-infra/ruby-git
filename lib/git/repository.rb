...
module Git
  ...
  class Repository
    ...
    # Open a working copy or bare repository at an existing path
    #
    ...
    #
    # @option options [String, nil] :git_ssh path to a custom SSH executable
    #
    # @option options [String] :binary_path path to the git binary
    #
    ...
    def self.open(working_dir, options = {})
      ...
      unless options[:repository]
        working_dir = PathResolver.root_of_worktree(
          working_dir,
    ...
    #   `:binary_path`, `:log`)
    #
    # @param paths [Hash{Symbol => (String, nil)}] the resolved `:working_directory`,
    #   `:repository`, and `:index` paths
    #
    ...
    #
    # Sums the sizes of every file under the repository (`.git`) directory.
    #
    ...
    def repo_size
      ...
      all_files = Dir.glob(File.join(repository.to_s, '**', '*'), File::FNM_DOTMATCH)

      all_files.map { |file| File.expand_path(file) }
               .uniq
